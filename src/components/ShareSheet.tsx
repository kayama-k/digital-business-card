...
export function ShareSheet({ open, onClose, onAction, returnFocusRef }: ShareSheetProps) {
  ...
  return (
    <div className="sheet-layer" role="presentation">
      <button className="sheet-layer__backdrop" aria-label="保存・シェアを閉じる" onClick={onClose} />
      <div ref={dialogRef} className="share-sheet" role="dialog" aria-modal="true" aria-labelledby="share-heading">
        ...
        <div className="share-sheet__actions">
          {actions.map((action) => (
            <button key={action.id} className="share-action" onClick={() => onAction(action.id)}>
        ...
        <button className="share-sheet__close" onClick={onClose}>閉じる</button>
      </div>
    </div>
  )
}
