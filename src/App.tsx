...
export default function App() {
  ...
  const setCardView = (nextView: CardView) => {
    ...
    waveTimerRef.current = window.setTimeout(() => setTransitioning(false), cardConfig.animation.waveDurationMs)
    setView(nextView)
  }

  const renderExport = useCallback(async () => {
    if (!exportRef.current) throw new Error('保存用カードを準備できませんでした。')
    return toPng(exportRef.current, { cacheBust: true, width: 1024, height: 1536, pixelRatio: 1 })
  }, [])
  ...
  const onShareAction = async (action: ShareAction) => {
    try {
      ...
      if (action === 'pdf') {
        ...
        const pdf = new jsPDF({ orientation: 'portrait', unit: 'px', format: [1024, 1536] })
      ...
      if (action === 'share') {
        if (navigator.share) {
          await navigator.share({ title: cardConfig.title, text: 'ただの、自己紹介。', url: cardConfig.urls.publicCard })
          showNotice('共有しました。')
        } else {
    ...
    <main className="app-shell">
      <article className={`business-card business-card--${view}`} style={{ '--page-duration': `${cardConfig.animation.pageDurationMs}ms`, '--page-easing': cardConfig.animation.easing, '--paper-texture': `url(${cardConfig.assets.paperTexture})` } as React.CSSProperties}>
        ...
        <div className="card-viewport" aria-live="polite">
          <div className={`card-track ${transitioning ? 'card-track--moving' : ''}`}>
        ...
        <nav className="bottom-nav" aria-label="名刺のメニュー">
          <button className={view === 'profile' ? 'is-current' : ''} aria-current={view === 'profile' ? 'page' : undefined} onClick={() => setCardView('profile')}>
            <span aria-hidden="true">●</span>プロフィール
          </button>
          <button className={view === 'portfolio' ? 'is-current' : ''} aria-current={view === 'portfolio' ? 'page' : undefined} onClick={() => setCardView('portfolio')}>
            <span aria-hidden="true">↗</span>ポートフォリオ
          </button>
          <button ref={shareButtonRef} onClick={() => setSheetOpen(true)}>
            <span aria-hidden="true">↓</span>保存・シェア
          </button>
        </nav>
      </article>

      <div className="export-stage"><ExportCard ref={exportRef} /></div>
      <ShareSheet open={sheetOpen} onClose={() => setSheetOpen(false)} onAction={onShareAction} returnFocusRef={shareButtonRef} />
      <p className="toast" role="status" aria-live="polite">{notice}</p>
    </main>
  )
}
